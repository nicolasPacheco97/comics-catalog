...
function CardComic({item, responsive = false}: Props){
    return <Link to={{
        pathname: `/${paths.base}/${paths.searchComic}`,
        search: `?id=${item.id}`
    ...
    <div className="w-full">
        <h3 className="font-marvel text-3xl text-carbon text-left uppercase tracking-wider">{item.title}</h3>
